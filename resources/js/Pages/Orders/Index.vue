<template>
    <app-layout>
        <template #header>
            <h1 class="text-center text-2xl font-bold leading-7 text-gray-300 sm:text-3xl sm:truncate py-4 bg-gradient-to-l from-indigo-500 to-indigo-800 ">LISTAR LAS ORDENES</h1>
        </template>
                    ...
                    <table class="rounded-t-lg mt-5 min-w-full mx-auto bg-gray-800 text-gray-100">
                        ...
                        <tr v-for="order in orders" :key="order.id">
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="ml-4">
                                <div class="text-sm font-medium">{{ order.tipo }}</div>
                                </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm"><span>$ </span>{{ order.cantidad }}</div>                    
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm"><span>$ </span>{{ order.estado }}</div>                    
                            </td>
                            <td class="flex m-2">
                            <inertia-link 
